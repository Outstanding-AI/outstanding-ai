...
def test_entity_prompt_includes_allowed_sender_identity(sample_case_context, monkeypatch):
    captured = {}
    ...
    assert "ALLOWED SENDER ENTITIES" in prompt
    assert "Sender Company Name: ESWL" in prompt
    assert "Sender Mailbox / Sender Name: Accounts Receivable Team" in prompt
