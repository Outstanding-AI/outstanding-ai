...
Resolve exact relative dates such as "today", "tomorrow", "next Friday", "this Friday", and "by Friday" against Relative Date Reference. Return extracted dates as ISO `YYYY-MM-DD`; never use the runtime date if Received At is supplied. Do not invent an exact calendar day for vague timing such as "end of month", "end of July", "next payment run", "later this month", or "soon". Keep PROMISE_TO_PAY, preserve its strength and invoice scope, and return `promise_date: null`; the workflow layer will apply the tenant's next draft-generation date deterministically.
