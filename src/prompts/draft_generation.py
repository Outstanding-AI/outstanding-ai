...
- When the normal collection basis is overdue, debtor-facing subject/body wording must say
  "overdue invoice(s)" or "overdue balance". Do NOT use open-balance synonyms that imply
...
- Prefer "Your invoice is overdue" over vague account-balance wording.
...
- Sign-off: use the mailbox name (e.g., "Kind Regards, Accounts Receivable Team"),
...
- Example: "Our accounts team has been in touch regarding your overdue invoices.
...
- partial_payment_ack: Warm, appreciative for the partial; clear, non-pressuring on the
  ...
  for the partial payment received. State the residual open balance clearly. Ask
...
6. One specific call-to-action appropriate to the conversation stage
7. Contact details for queries only if they are explicitly needed
8. Professional sign-off:
   - Prefer the sales-ledger sign-off: "Thank you" followed by "Kind Regards,"
     and the actual sender/team name.
...
Subject Line Style:
- Subject lines should be factual and short. When replying, preserve the existing thread subject.
- For sales-ledger/shared-mailbox collection, prefer concrete subjects such as
  "Invoice {ref}", "Follow-up: Overdue Invoices for {company}", or the existing
  debtor thread subject over clever or conversational rewrites.
...
- Signature should be formatted as: <p>Thank you</p><p>Kind Regards,</p><p>{actual sender/team name}</p>
