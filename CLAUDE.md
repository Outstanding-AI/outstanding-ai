...
**Long-term home**: backend-owned `solvix-contracts` package (`Solvix/contracts/src/solvix_contracts/ai/context.v2`) exports the shared v2 core while AI carries additive v3/v4 transition fields locally. Backend bumped `solvix-contracts` to **0.11.0** (`contracts/pyproject.toml`); AI still pins **`solvix-contracts==0.9.0`** in `pyproject.toml` — parity CI at `.github/workflows/contracts-version-parity.yml` will block any joint deploy until the AI pin is aligned. Bumping the AI pin requires coordinated review (do not bump unilaterally).
