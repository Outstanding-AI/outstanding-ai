...
`POST /select-collection-chain` is the dedicated debtor-grain router used only
...
- **Fallback: OpenAI** (`gpt-5.6-luna`, temperature 0.3, LangChain `ChatOpenAI`). Disabled if it equals the primary or `OPENAI_API_KEY` is unset. AI code defaults Luna to `medium` reasoning; callers can override it per request, and legacy `none` is normalized to the live endpoint's lowest supported `minimal` effort. The setting is intentionally not an environment variable. Same tenacity retry shape. **No application-level max-token cap** is set (`OPENAI_MAX_TOKENS`/`VERTEX_MAX_TOKENS` env vars were removed 2026-04-29).
