...
- **Fallback: OpenAI** (`gpt-5.6-luna`, temperature 0.3, LangChain `ChatOpenAI`). Disabled if it equals the primary or `OPENAI_API_KEY` is unset. The optional reasoning parameter is omitted by default; legacy `none` is normalized to the live endpoint's lowest supported `minimal` effort. Same tenacity retry shape. **No application-level max-token cap** is set (`OPENAI_MAX_TOKENS`/`VERTEX_MAX_TOKENS` env vars were removed 2026-04-29).
